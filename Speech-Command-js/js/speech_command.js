...
$( document ).ready(function() {
    wordListMCQ = ["one","two","three","four"];
    wordListYRN= ["yes","no"];

    $.each(wordListMCQ, function( index, word ) {
        if (!word.startsWith('_')){
            $("#candidate-words-mcq").append(`<span class='candidate-word col-md-2 col-sm-3 col-3' id='word-${word}'>${word}</span>`);
        }
    });

    $.each(wordListYRN, function( index, word ) {
        if (!word.startsWith('_')){
            $("#candidate-words-yrn").append(`<span class='candidate-word col-md-2 col-sm-3 col-3' id='word-${word}'>${word}</span>`);
        }
    });
});
...
function loadModel(){
    ...
    Promise.all([
        ...
        $.each(words, function( index, word ) {
            if (!word.startsWith('_') && !wordListMCQ.includes(word)){
